...
let logger = function () {
	...
	console.log(
		"Даныне клиента: " + window.sessionStorage.getItem("userAgent")
	);
	console.log(
		"Возраст пользователя: " + window.sessionStorage.getItem("userAge")
	);
};
...
function filterContent() {
	...
	for (let i = 0; i <= elements.length; i++) {
		...
		if (
			!videoText
				.toLowerCase()
				.includes(inputParseFunction().toLowerCase())
		) {
